...
class Editor extends Component {
  ...
  highlightText = () => {
    ...
    if (selectedText !== "") {
      ...
      console.log("outside arraymap");
      if (array.length === 0) {
        array.push(insertObj);
        console.log("array length is 0- so pushed the array-", array);
      } else {
        array.map((obj, index) => {
          console.log("obj", obj);
          if (cursorStart < obj.start) {
            if (addArray === false) {
              ...
              console.log("cursor less than the already one -", array);
              addArray = true;
            }
          }
        });
      ...
      for (let i = 0; i < array.length; i++) {
        if (i === 0 && array.length === 1) {
          console.log(this.state.textareaVal.substring(array[i].end));
